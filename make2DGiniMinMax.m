function [angleList, allGini] = make2DGiniMinMax(x, clusterID, clusterName, ratio)
% make the localy-scaled 2D Gini curve plot on the 2d data
%
% input:
...
for cluster = 1 : numCluster
    ...
    for i = 1 : length(angleList)
        ...
        gini(i) = ratio(cluster)*computeGini( ones(length(value), 1), value ) * (max(value)-min(value));
